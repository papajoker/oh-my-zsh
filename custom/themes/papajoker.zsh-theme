...
prompt_status() {
  ...
  #[[ $UID -eq 1000 ]] && symbols+="%{%F{yellow}%}☻"

  [[ -n "$symbols" ]] && prompt_segment black default "$symbols"
}

# machine, linux, dd system, dd home
prompt_linux() {
  couls=(28 34 40 46)
  prompt_segment $couls[1] black '%M'
  prompt_segment $couls[2] black $(lsb_release -is)
  prompt_segment $couls[3] black $(df -h -t ext4 |grep ^/  |  head -1 | cut -d "G" -f 3)
  prompt_segment $couls[2] black $(df -h -t ext4 |grep "/home/"  |  head -2 | cut -d "G" -f 2)
  prompt_end
}
